...
internal object NaturalComparator : Comparator<String> {
  ...
  private fun naturalCompare(
    ...
    while (i < length1 && j < length2) {
      ...
      if ((isDecimalDigit(ch1) || ch1 == ' ') && (isDecimalDigit(ch2) || ch2 == ' ')) {
        ...
        if (ch1 != ch2) {
          val diff: Int = when {
            ch1 == '-' && ch2 != '_' -> {
              compareChars(collator, '_', ch2)
            }
            ch2 == '-' && ch1 != '_' -> {
              compareChars(collator, ch1, '_')
            }
            else -> {
              compareChars(collator, ch1, ch2)
            }
          }
          if (diff != 0) return diff
        }
      } else {
        val diff = compareChars(collator, ch1, ch2)
        if (diff != 0) return diff
      }
      i++
      j++
    ...
    return if (ignoreCase) naturalCompare(collator, s1, s2, length1, length2, false, likeFileNames) else 0
  }
