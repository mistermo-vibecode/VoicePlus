...
class PlayerController(
  ...
  suspend fun currentPlaybackSnapshot(): Pair<BookId?, Boolean>? {
    val controller = awaitConnect() ?: return null
    return controller.currentBookId() to controller.isPlaying
  }

  fun playPauseForBook(bookId: BookId) {
    scope.launch {
      val controller = awaitConnect() ?: return@launch
      val playingId = controller.currentBookId()
      when {
        playingId == bookId && controller.isPlaying -> controller.pause()
        playingId == bookId && !controller.isPlaying -> {
          if (maybePrepare(controller)) controller.play()
        }
        else -> {
