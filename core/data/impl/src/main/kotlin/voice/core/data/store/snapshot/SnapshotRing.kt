...
internal class SnapshotRing(private val slots: List<DataStore<LibrarySnapshot?>>) {
  ...
  suspend fun readAll(): List<LibrarySnapshot> =
    slots.mapNotNull { slot -> runCatching { slot.data.first() }.getOrNull() }

  suspend fun best(): LibrarySnapshot? =
    readAll().filter { it.activeCount > 0 }.maxByOrNull { it.sequence }
